...
function dockerDownNote() {
  ...
  if (/^tcp:\/\//i.test(host)) {
    return `Nothing is answering at <code class="mono">${escapeHtml(host)}</code>. <code class="mono">DOCKER_HOST</code> points there, so check that the configured socket proxy is running and reachable from Companion. Restart it from Unraid or its Compose project.`;
  }
  const path = host.replace(/^unix:\/\//, '') || '/var/run/docker.sock';
  return `No Docker socket at <code class="mono">${escapeHtml(path)}</code>. Mount <code class="mono">${escapeHtml(path)}:${escapeHtml(path)}:ro</code> on Companion, then recreate the container.`;
}
